...
contains
  ...
  !-----------------------------------------------------------------|
    ...
    use modglobal, only : itot,jtot, ysize,xsize,dtmax,runtime, startfile,lwarmstart,eps1, imax,jmax, ladvectonly, lcoldstartfiles
  ...
  subroutine readinitfiles
    ...
    use modmpi,            only : slabsum,myid,comm3d,mpierr,D_MPI_BCAST,myidx,myidy,cmyid
    ...
    allocate(qtcinfield (itot,jtot,kmax))
          ...
          do j=1,jtot
            do i=1,itot
              if (i >= is .and. i <= ie .and. &
                  j >= js .and. j <= je) then
                thl0(i-is+2,j-js+2,k)=thladvfield(i,j,k)
                thlm(i-is+2,j-js+2,k)=thladvfield(i,j,k)
                qt0(i-is+2,j-js+2,k)=qtadvfield(i,j,k)
                qtm(i-is+2,j-js+2,k)=qtadvfield(i,j,k)
              end if
            end do
          end do
        end do
        ...
        do k=1,kmax
          do j=1,jtot
            do i=1,itot
              if (i >= is .and. i <= ie .and. &
                  j >= js .and. j <= je) then
                thl0(i-is+2,j-js+2,k)=thlcinfield(i,j,k)
                thlm(i-is+2,j-js+2,k)=thlcinfield(i,j,k)
                qt0(i-is+2,j-js+2,k)=qtcinfield(i,j,k)
                qtm(i-is+2,j-js+2,k)=qtcinfield(i,j,k)
              end if
            end do
          end do
        end do
...
!******include rho if rho = rho(z) /= 1.0 ***********
  ...
  !******include rho if rho = rho(z) /= 1.0 ***********
    ...
    deallocate (thladvfield,qtadvfield)
    deallocate (thlcinfield,qtcinfield)
