...
!> Initializing Timestat. Read out the namelist, initializing the variables
  subroutine inittimestat
    ...
    dtav,ltimestat,blh_thres,iblh_meth,iblh_var,blh_nsamp,llwp_pos,ips1,ips2,ips3,ips4,jps1,jps2,jps3,jps4 !! namelist contents
    ...
    call D_MPI_BCAST(ips3  ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(ips4  ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(jps1  ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(jps2  ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(jps3  ,1,0,comm3d,mpierr)
    call D_MPI_BCAST(jps4  ,1,0,comm3d,mpierr)
    idtav = dtav/tres
    ...
    if(myid==0) then
       if (.not. lwarmstart) then
          ...
          if (llwp_pos) then
            ...
            close(ifoutput)
            open (ifoutput,file='tmradps2.'//cexpnr,status='replace',position='append')
            write(ifoutput,'(2a)') &
                 '#  time      LWP_ps2      SWD_ps2'
            close(ifoutput)
            open (ifoutput,file='tmradps3.'//cexpnr,status='replace',position='append')
            write(ifoutput,'(2a)') &
                 '#  time      LWP_ps3      SWD_ps3'
            close(ifoutput)
            open (ifoutput,file='tmradps4.'//cexpnr,status='replace',position='append')
            write(ifoutput,'(2a)') &
                 '#  time      LWP_ps4      SWD_ps4'
            close(ifoutput)
          end if
          if(isurf == 1) then
...
!>Run timestat. Calculate and write the statistics
  subroutine timestat
    ...
    use modglobal,  only : i1,j1,kmax,zf,dzf,cu,cv,rv,rd,eps1,imax,jmax,&
    ...
    use modmpi,     only : mpi_sum,mpi_max,mpi_min,comm3d,mpierr,myid, D_MPI_ALLREDUCE,myidx,myidy,D_MPI_BCAST
    ...
    real   :: LWP1, LWP2, LWP3, LWP4, SWD1, SWD2, SWD3, SWD4
  ...
  !     --------------------------------------------------------------
    ...
    !call D_MPI_BCAST(qlint_field,(i1-2)*jtot,0,comm3d,mpierr)
  ...
  !     ---------------------------------------
    ...
    if(myid == 0) then
      open (ifoutput,file='tmrad.'//cexpnr,position='append')
      write( ifoutput,'(f10.2)',advance='no') &
            rtimee
      close(ifoutput)
    end if
    ...
    if (llwp_pos) then
      ...
      if (ips2 >= is .and. ips2 <= ie .and. jps2 >= js .and. jps2 <= je) then
        LWP2 = 0.0
        do k=1,kmax
          LWP2 = LWP2 + ql0(ips2-is+2,jps2-js+2,k)*rhof(k)*dzf(k)
        end do
        SWD2 = swd(ips2-is+2,jps2-js+2,1)
        open (ifoutput,file='tmradps2.'//cexpnr,position='append')
        write( ifoutput,'(f10.2,2e13.4)',advance='no') &
              rtimee,&
              LWP2,&
              SWD2
      ...
      if (ips3 >= is .and. ips3 <= ie .and. jps3 >= js .and. jps3 <= je) then
        LWP3 = 0.0
        do k=1,kmax
          LWP3 = LWP3 + ql0(ips3-is+2,jps3-js+2,k)*rhof(k)*dzf(k)
        end do
        SWD3 = swd(ips3-is+2,jps3-js+2,1)
        open (ifoutput,file='tmradps3.'//cexpnr,position='append')
        write( ifoutput,'(f10.2,2e13.4)',advance='no') &
              rtimee,&
              LWP3,&
              SWD3
      ...
      if (ips4 >= is .and. ips4 <= ie .and. jps4 >= js .and. jps4 <= je) then
        LWP4 = 0.0
        do k=1,kmax
          LWP4 = LWP4 + ql0(ips4-is+2,jps4-js+2,k)*rhof(k)*dzf(k)
        end do
        SWD4 = swd(ips4-is+2,jps4-js+2,1)
        open (ifoutput,file='tmradps4.'//cexpnr,position='append')
        write( ifoutput,'(f10.2,2e13.4)',advance='no') &
              rtimee,&
              LWP4,&
              SWD4
